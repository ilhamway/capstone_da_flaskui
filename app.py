...
def index():
    ...
    content_rating = df2.groupby('Content Rating').agg({
    'Content Rating' : 'count'
    }).rename({'Content Rating':'Total'}, axis=1).sort_values(by='Total', ascending=False).head().reset_index()
    X = content_rating['Content Rating'].values
    Y = content_rating['Total'].values
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
    plt.bar(X,Y)
    plt.xlabel('Rating')
